...
import com.edgareldy.springbatchtutorial.dto.common.PageResponse;
 ...
 * listing run history, and resuming a run that was flagged
 * {@code AWAITING_REVIEW} by launching {@code payrollFinalizeJob} once a
 * human has reviewed it out-of-band.
 ...
 * {@code GET /api/v1/payroll/runs} returns
 * {@code ApiResponse<PageResponse<PayrollRunResponse>>}, letting an admin
 * browse run history without querying {@code BATCH_JOB_EXECUTION} directly.
 * <p>
...
public class PayrollController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<PageResponse<PayrollRunResponse>>> listRuns(
            @PageableDefault(size = 20, sort = "startedAt", direction = Sort.Direction.DESC) Pageable pageable) {
        Page<PayrollRunResponse> runs = payrollRunService.listRuns(pageable).map(this::toResponse);
        return ResponseEntity.ok(ApiResponse.success(PageResponse.from(runs), "Payroll runs retrieved"));
    }
